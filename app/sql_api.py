...
def add_flower(mainphoto, images, square, name, year):
    conn, cursor = get_db()
    cursor.execute("INSERT INTO flower (mainphoto, images, square, name, year) VALUES (?, ?, ?, ?, ?)",
                   (mainphoto, images, square, name, year))
    conn.commit()
    cursor.close()
    conn.close()
    return "Flower added"
